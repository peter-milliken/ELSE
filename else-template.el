...
(defun else-dump-definition-header (definition)
  ...
  (let ((name (slot-value definition 'name))
    ...
    (mapc #'(lambda (s)
              ...
              (if (string-match-p "\\s-" (slot-value definition 'language-name))
                  (insert (concat "/LANGUAGE=\"" (slot-value definition 'language-name) "\""))
                (insert (concat "/LANGUAGE=" (slot-value definition 'language-name))))
              (newline))
          delete-define-type)))
...
(cl-defmethod dump ((obj else-placeholder-base))
  ...
  (let ((insert-position (point))
        ...
        (formatted-string nil))
    (message "base")
    (if (slot-value obj 'reference)
        (progn
          (indent-to indent-column)
          (insert (format "/PLACEHOLDER=\"%s\"" (slot-value obj 'reference))))
      (indent-to indent-column)
      (if (eq (slot-value obj 'substitution) 'noauto-substitute)
          (insert "/NOAUTO_SUBSTITUTE")
        (insert "/AUTO_SUBSTITUTE")
        (newline)
        (indent-to indent-column)
        (insert (concat "/SUBSTITUTE_COUNT=" (number-to-string (slot-value obj 'substitution-count)))))
      (newline)
      (indent-to indent-column)
      (insert (format "/DESCRIPTION=\"%s\"" (slot-value obj 'description)))
      (newline)
      (indent-to indent-column)
      (insert (format "/DUPLICATION=%s" (cl-case (slot-value obj 'duplication)
                                          ('context-dependent "CONTEXT_DEPENDENT")
                                          ('vertical "VERTICAL")
                                          ('horizontal "HORIZONTAL"))))
      (newline)
      (indent-to indent-column)
      (insert (format "/SEPARATOR=\"%s\"" (slot-value obj 'separator)))
      (newline))))
...
(cl-defmethod dump ((obj else-non-terminal-placeholder) tab-size)
  ...
  (let ((indent-column 4)
    ...
    (if (not (slot-value obj 'reference))
        (progn
          (indent-to indent-column)
...
(cl-defmethod dump ((obj else-menu-placeholder) tab-size)
  ...
  (let ((indent-column 4))
    ...
    (insert "/TYPE=MENU")
    (newline)
    (dolist (this-line (slot-value obj 'menu))
      (newline)
      (indent-to indent-column)
...
(cl-defmethod dump ((obj else-terminal-placeholder) tab-size)
  ...
  (let ((indent-column 4))
    ...
    (newline)
    (dolist (this-line (slot-value obj 'prompt))
      (newline)
      (indent-to indent-column)
...
(cl-defmethod dump ((obj else-language))
  ...
  (let ((index nil)
    ...
    (insert (format "DELETE LANGUAGE \"%s\"" (slot-value obj 'name)))
    (newline)
    (insert (format "DEFINE LANGUAGE \"%s\"" (slot-value obj 'name)))
    (newline)
    (indent-to indent-column)
    (insert (format "/INITIAL_STRING=\"%s\"" (slot-value obj 'initial-string)))
    (newline)
    (map-char-table #'(lambda (key value)
                    ...
                    (slot-value obj 'punctuation-characters))
    (indent-to indent-column)
    (insert (format "/PUNTUATION_CHARACTERS=\"%s\"" p-chars))
    (newline)
    (indent-to indent-column)
    (insert (format "/INDENT_SIZE=%s" (slot-value obj 'tab-size)))
    (newline)
    (indent-to indent-column)
    (insert (format "/VERSION=%s" (slot-value obj 'version)))
    (newline)
    (else-dump-end-define)
    (newline)
    (dolist (name (get-names obj))
      (dump (symbol-plist (intern-soft name (slot-value obj 'placeholders))) (slot-value obj 'tab-size)))))
